...
class MSCI0E(nn.Module):
    '''
    添加doc（相当于结合DeepConn）
    '''

    def __init__(self, opt):
        super(MSCI0E, self).__init__()
        self.opt = opt
...
class Net(nn.Module):
    ...
    def forward(self, doc, reviews, ids, ids_list, sentiments):  # 添加了sentiments
        ...
        doc_fea = F.relu(self.cnn_doc(doc.unsqueeze(1))).squeeze(3)
        ...
        return torch.stack([F.relu(id_emb), F.relu(doc_fea), F.relu(self.fc_layer(r_fea))], 1)
