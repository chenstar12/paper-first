...
'''
继续简化：删除另一个doc(相当于NARRE减去所有id embedding)
'''


class MSCI11D(nn.Module):

    def __init__(self, opt):
        super(MSCI11D, self).__init__()
        self.opt = opt
...
class Net(nn.Module):
    def __init__(self, opt, uori):
        ...
        self.mix_layer = nn.Linear(self.opt.filters_num, self.opt.filters_num)
    ...
    def forward(self, doc, reviews, ids, ids_list, sentiments):  # 添加了sentiments
        ...
        r_fea = F.leaky_relu_(self.mix_layer(r_fea))  # 还是[128,100]
        '''
        是否需要relu
        '''
