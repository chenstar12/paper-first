...
class Net(nn.Module):
    def __init__(self, opt, uori):
        ...
        self.ui_linear = nn.Linear(self.opt.filters_num, self.opt.id_emb_size)
    ...
    def forward(self, doc, reviews, ids, ids_list, sentiments):  # 添加了sentiments
        ...
        if self.uori == 'user':
            u_i_id_emb = F.leaky_relu_(self.cnn_u_id(u_i_id_emb.unsqueeze(1))).squeeze(
                2)  # [1280,32]->[1280,1,32]->[1280,100,1] -> [1280,100]
        else:
    ...
    def reset_para(self):
        ...
        nn.init.xavier_normal_(self.cnn_u_id.weight)
        nn.init.constant_(self.cnn_u_id.bias, 0.1)
        nn.init.xavier_normal_(self.cnn_i_id.weight)
        nn.init.constant_(self.cnn_i_id.bias, 0.1)

        # nn.init.uniform_(self.id_linear.weight, -0.1, 0.1)

        nn.init.uniform_(self.linear.weight, -0.1, 0.1)
        nn.init.constant_(self.linear.bias, 0.1)
        nn.init.uniform_(self.ui_linear.weight, -0.1, 0.1)
        nn.init.constant_(self.ui_linear.bias, 0.1)
